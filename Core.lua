...
-- Macro mode: a smartAction can render as a generated, per-character macro instead of
-- placing the resolved spell/item directly, so users can wrap it in mouseover /
-- focus / conditional logic while the smartAction still resolves the right action.
...
-- after their smartAction and suffix this marker so we can recognise (and safely clean
-- up) our own macros even if the tracking registry is ever lost. The smartAction name
