...
-- Macro mode: a memory can render as a generated, per-character macro instead of
-- placing the resolved spell/item directly, so users can wrap it in mouseover /
-- focus / conditional logic while the memory still resolves the right action.
...
-- Macro names cap at 16 characters; we name generated macros after their memory
-- and suffix this marker so we can recognise (and safely clean up) our own macros
-- even if the tracking registry is ever lost. The memory name is truncated to fit.
