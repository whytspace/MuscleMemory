...
describe("Actions", function()
  ...
  describe("GetAssignmentName", function()
    it("returns the bare name without id suffixes or type prefixes", function()
      stubs:setSpell(1766, { name = "Kick" })
      stubs:setItem(6948, { name = "Hearthstone" })
      assert.equals("Kick", MM.Actions.GetAssignmentName({ type = "spell", id = 1766 }))
      assert.equals("Hearthstone", MM.Actions.GetAssignmentName({ type = "item", id = 6948 }))
      assert.equals("Heal", MM.Actions.GetAssignmentName({ type = "macro", nameHint = "Heal" }))
      assert.equals("Tank", MM.Actions.GetAssignmentName({ type = "equipmentset", name = "Tank" }))
    end)

    it("falls back to the full label when no clean name is available", function()
      assert.equals("Spell ID: 999", MM.Actions.GetAssignmentName({ type = "spell", id = 999 }))
      assert.equals("Empty", MM.Actions.GetAssignmentName({ type = "empty" }))
    end)
  end)
