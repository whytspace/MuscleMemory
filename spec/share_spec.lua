...
describe("Share", function()
  ...
  -- A profile with two layers and two custom smart actions; the "Raid" layer
  ...
  local function seedContent()
    ...
    profile.layers.Core.slots = {
      ...
      [2] = { type = "action", source = "custom", id = "burst" },
    }
    profile.layers.Raid = {
      name = "Raid",
      enabled = true,
      slots = {
        [1] = { type = "action", source = "custom", id = "heal" },
        [2] = { type = "action", source = "predefined", id = "interrupt" },
  ...
  describe("BuildPackage", function()
    it("packs the whole profile including settings when no selection is given", function()
      ...
      assert.is_table(package.actions.burst)
      assert.is_table(package.actions.heal)
      assert.same({ fallback = "keep", response = "popup" }, package.settings)
    end)

    it("always bundles the custom smart actions a selected layer references", function()
      local profileId = seedContent()
      ...
      assert.is_table(package.actions.heal)
      assert.is_nil(package.actions.burst)
      assert.is_nil(package.settings)
    end)

    it("exports smart actions on their own", function()
      local profileId = seedContent()
      local package = MM.Share:BuildPackage(profileId, { actions = { burst = true } })

      assert.equals(0, #package.layers)
      assert.is_table(package.actions.burst)
      assert.is_nil(package.actions.heal)
    end)
    ...
    it("copies data instead of referencing profile tables", function()
      ...
      local package = MM.Share:BuildPackage(profileId)
      package.actions.burst.name = "Changed"
      assert.equals("Burst", profile.actions.burst.name)
    end)
  end)
  ...
  describe("Encode/Decode", function()
    it("round-trips a package through the printable string", function()
      ...
      assert.truthy(text:match("^!MM:2!"))
      assert.truthy(text:match("^!MM:2![%w%(%)]*$"))
    ...
    -- A v1 string predates the smart-action rename; its field names and slot
    -- discriminator are lifted to the current ones instead of being rejected.
    it("upgrades a v1 package on decode", function()
      local package = {
        version = 1,
        schemaVersion = 3,
        profileName = "Old",
        layers = {
          {
            key = "Core",
            layer = { name = "Core", slots = { [1] = { type = "dynamicaction", source = "custom", id = "burst" } } },
          },
        },
        dynamicActions = { burst = { name = "Burst", candidates = {} } },
      }
      -- Encode stamps the current prefix; only the header states the version.
      local text = MM.Share:Encode(package):gsub("^!MM:2!", "!MM:1!")

      local decoded = MM.Share:Decode(text)
      assert.is_nil(decoded.dynamicActions)
      assert.equals("Burst", decoded.actions.burst.name)
      assert.equals("action", decoded.layers[1].layer.slots[1].type)
    end)
  ...
  describe("Import", function()
    it("imports everything into the current profile with fresh keys", function()
      ...
      local profile = MM.DB:GetProfile(profileId)
      assert.equals("Burst", profile.actions.burst.name)
      assert.equals("Burst", profile.actions.burst_2.name)
      assert.equals("Heal", profile.actions.heal_2.name)
      ...
      assert.same({ profileId = profileId, layers = { "core", "raid" }, actions = { "burst_2", "heal_2" } }, {
    ...
    it("rewrites imported layer slots to the re-keyed smart actions", function()
      local profileId = seedContent()
      local package = MM.Share:BuildPackage(profileId)
      MM.Share:Import(package, nil, { profileId = profileId })

      local profile = MM.DB:GetProfile(profileId)
      assert.same({ type = "action", source = "custom", id = "burst_2" }, profile.layers.core.slots[2])
      assert.same({ type = "action", source = "custom", id = "heal_2" }, profile.layers.raid.slots[1])
      -- Predefined references and macros pass through untouched.
      assert.same({ type = "action", source = "predefined", id = "interrupt" }, profile.layers.raid.slots[2])
    ...
    it("imports a partial selection and keeps layer dependencies", function()
      ...
      MM.Share:Import(package, { layers = { Raid = true }, actions = {} }, { profileId = profileId })

      local profile = MM.DB:GetProfile(profileId)
      assert.is_table(profile.layers.raid)
      assert.is_nil(profile.layers.core)
      assert.is_table(profile.actions.heal_2)
      assert.is_nil(profile.actions.burst_2)
    end)
    ...
    it("imports into a new profile with the package settings", function()
      ...
      assert.same({ type = "action", source = "custom", id = "heal" }, imported.layers.raid.slots[1])
    end)
    ...
    it("rejects an empty selection", function()
      local profileId = seedContent()
      local package = MM.Share:BuildPackage(profileId)
      local result, reason = MM.Share:Import(package, { layers = {}, actions = {} }, { profileId = profileId })
    ...
    it("marks imported entities for the session pills", function()
      ...
      assert.is_true(MM.Share:IsRecentImport("actions", result.profileId, "burst"))
