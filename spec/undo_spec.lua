...
describe("Undo", function()
  ...
  it("groups a batch into one step", function()
    MM.Undo:Batch(function()
      local key = MM.DB:CreateDynamicAction("Defensives")
      MM.DB:AddCandidate(key, { type = "spell", id = 498 })
      local layerId = MM.DB:CreateLayer("Healing")
      MM.DB:SetSlot(layerId, 5, { type = "dynamicaction", source = "custom", id = key })
    end)

    assert.is_true(MM.Undo:Undo())
    assert.is_nil(next(MM.DB:DynamicActions()))
  ...
  describe("focus", function()
    ...
    it("jumps to the affected dynamic action and candidate", function()
      local key = MM.DB:CreateDynamicAction("Defensives")
      MM.DB:AddCandidate(key, { type = "spell", id = 498 })
      ...
      MM.Undo:Undo()
      assert.equals("dynamicActions", MM.ui.state.tab)
      assert.same({ source = "custom", id = key }, MM.ui.state.dynamicAction)
